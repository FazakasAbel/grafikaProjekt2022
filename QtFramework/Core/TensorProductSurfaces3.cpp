...
TensorProductSurface3::TensorProductSurface3(
        ...
        GLboolean u_closed, GLboolean v_closed) : _u_min(u_min), _u_max(u_max),
...
TensorProductSurface3::TensorProductSurface3(const TensorProductSurface3& surface): _u_min(surface._u_min), _u_max(surface._u_max),
                                                                                    _v_min(surface._v_min), _v_max(surface._v_max),
                                                                                    _data(surface._data), _u_closed(surface._u_closed), _v_closed(surface._v_closed)
{
}
    ...
    for (GLuint i = 0; i < iso_line_count; i++) {
        ...
        for (GLuint j = 0; j < div_point_count; j++) {
           ...
           for (GLuint k = 0; k < i; k++) {
                ...
                throw Exception("Could not calculate the partial derivatives");
                delete result, result = nullptr;
                return result;
            }
...
GLboolean TensorProductSurface3::UpdateVertexBufferObjectsOfData(GLenum usage_flag)
{
    ...
    GLuint data_size = 2 * _data.GetRowCount() * _data.GetColumnCount() * 3 * sizeof(GLfloat);
