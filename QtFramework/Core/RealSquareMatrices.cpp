...
{
    return Matrix::ResizeRows(row_count) && Matrix::ResizeColumns(row_count);
}

GLboolean RealSquareMatrix::ResizeColumns(GLuint column_count)
{
    return Matrix::ResizeRows(column_count) && Matrix::ResizeColumns(column_count);
}

// homework: copy constructor
RealSquareMatrix::RealSquareMatrix(const RealSquareMatrix& m) : Matrix<GLdouble>(m._row_count, m._column_count),
